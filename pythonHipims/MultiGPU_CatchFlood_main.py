...
def ave_index_ToDevice(CELL_MASK, gauge_index_1D, rank, size):
    ...
    if rank == 0:
        ...
        print("Rank ", rank)
        gaugeGlobalIndex = torch.flatten(
            ((gauge_index_1D >= number_begin)
             & (gauge_index_1D < number_end)).nonzero()).type(torch.int32)
        print(gaugeGlobalIndex)
...
def run(rank, size, paraDict):
    ...
    if rank == 0:
        ...
        dt_list = []
        t_list = []
    dem_main, cell_type_mask_main, demMeta, gauge_index_1D = pre.importDEMData_And_BC(
    ...
    landuse_main, landuse_index = pre.importLanduseData(
    ...
    rainfall_station_Mask_main = pre.importRainStationMask(
    ...
    numerical.set__infiltrationField_tensor(paraDict['hydraulic_conductivity'],
                                      paraDict['capillary_head'], paraDict['water_content_diff'], 
                                      paraDict['device'])

    # # DEBUG
    # print("Rank: %d, have %d cells", rank, h.size()[0])
    # print(observe_gauge_index)
    # print(observe_gauge_index.size()[0])
    # quit()
    ...
    if gauge_index_1D.size()[0] > 0:
        ...
        while numerical.t.item() < paraDict['EndTime']:
            ...
            numerical.observeGauges_write(observe_gauge_index,
                                          gauge_dataStoreList, n)
            numerical.addFlux()
    ...
    np.savetxt(paraDict['OUTPUT_PATH'] + '/' + str(rank) + '_gauges.txt',
               gauge_dataStoreList)
    if rank == 0:
...
def multiGPU_floodSimulator(run, paraDict, backend='nccl'):
...
if __name__ == "__main__":
    ...
    multiGPU_floodSimulator(run, paraDict, backend='nccl')
    post.multi_exportRaster_tiff(rasterPath['DEM_path'],
                                 paraDict['OUTPUT_PATH'], 2)
