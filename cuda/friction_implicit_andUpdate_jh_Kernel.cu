      ...
      if (q_norm > 1.0e-10)
      {
        if (abs(qx[i]) > 1.0e-10)
        {
          ...
          if (manning[landuse[i]] > 0.0)
          {
            ...
            // if (abs(temp) > 1.0e-10)
            // {
            //   qx[i] = abs(temp) / temp * min(abs(temp), 5.0 * h[i] * sqrt(h[i] * g));
            // }
            // else
            // {
            //   qx[i] = 0.0;
            ...
            // if fr>5.0, add friction/2 again
            if (abs(temp) > 5.0 * h[i] * sqrt(h[i] * g))
            {
              // qx[i] = 1.5 * temp - 0.5 * qx[i];
              qx[i] = 2.0 * temp - qx[i];
        ...
        if (abs(qy[i]) > 1.0e-10)
        {
          ...
          if (manning[landuse[i]] > 0.0)
          {
            ...
            // if (abs(temp) > 1.0e-10)
            // {
            //   qy[i] = abs(temp) / temp * min(abs(temp), 5.0 * h[i] * sqrt(h[i] * g));
            // }
            // else
            // {
            //   qy[i] = 0.0;
            ...
            // ==================================================//
            // add a limit for the supercritical flow
            // if fr>5.0, add friction again

            if (abs(temp) > 5.0 * h[i] * sqrt(h[i] * g))
            {
              // qy[i] = 1.5 * temp - 0.5 * qy[i];
              qy[i] = 2.0 * temp - qy[i];
