...
__global__ void fluxMask_kernel(const int N, bool *__restrict__ fluxMask,
  ...
  if (i < N) {
    if (t[0] == 0.0) {
      ...
      fluxMask[i] = false;
      fluxMask[i] = fluxMask[i] || (h[i] > h_small) || (lidMask[i] >= 1); //
      for (int nei = 1; nei < 5; nei++) {
        n_index = nei * N + i;
        n_index = index[n_index];
        if (n_index > 0) {
          fluxMask[i] = fluxMask[i] || (h[n_index] > h_small)|| (lidMask[n_index] >= 1); //
        }
      }
    }
  }
...
//  
void fluxMask_cuda(at::Tensor fluxMask, at::Tensor h, at::Tensor index, at::Tensor lidMask,
                   at::Tensor t) {
  ...
  AT_DISPATCH_FLOATING_TYPES(
      h.type(), "fluxMask_cuda", ([&] {
        fluxMask_kernel<scalar_t><<<block_0, thread_0, 0, stream>>>(
            ...
            lidMask.data<uint8_t>(), t.data<scalar_t>());
      }));
...
//           fluxMask[i] = fluxMask[i] || (boundaryType == 6);
//         }
//       }
//     }
//   }
